...
class InvertedTriPendulumEnv_swingup(MujocoEnv, utils.EzPickle):
    """
    ## Description
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, **kwargs):
        ...
        # xmlから時間ステップを抽出 [s]
        tree = ET.parse('./inverted_tri_pendulum.xml')
        root = tree.getroot()

        for value in root.iter('option'):
            print( value.attrib)

        self.d_time = float( value.attrib['timestep'])
        self.time = 0

    def step(self, action):

        self.time = self.time + self.frame_skip*self.d_time
        ...
        # dist_penalty = 0.1 * x**2 + (y - 2) ** 2
        # v1, v2, v3 = self.data.qvel[1:4]
        # vel_penalty = 7e-2 * v1**2 + 1e-2 * v2**2 + 1e-2 * v3**2
        # alive_bonus = 10
        
        # r = alive_bonus - dist_penalty - vel_penalty

        x_pos = self.data.qpos[:1]             # cart x pos [m]

        theta1, theta2, theta3 = self.data.qpos[1:4] # [rad]
        omega1, omega2, omega3 = self.data.qvel[1:4] # [rad/s]
        ...
        # terminated = bool(y <= 1 or self.time > 50)
        terminated = bool( self.time > 25.0 )
    ...
    def reset_model(self):
        ...
        self.set_state(
            self.init_qpos
            + np.concatenate([ 
                self.np_random.uniform(low=-2.0, high=2.0, size=1),                             # x position [m]
                self.np_random.uniform(low=-np.pi/4, high=np.pi/4, size=(self.model.nq-1))      # link angles [rad]
                ]),       
            self.init_qvel + self.np_random.standard_normal(self.model.nv) * 0.1,               # general velocity
        )
